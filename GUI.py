    ...
    try:
        ...
        X_dummy = pd.DataFrame(
            ...
            columns=[
                ...
                "Volume (L)",
                "Concentration of biochar in water(g/L)",
...
def compute_permutation_importance_cached(feature_names, feature_ranges, n_samples, seed=42):
    """
    返回始终为“降序”的 perm importance（最重要在最上面）
    注意：不在这里写 session_state，避免副作用。
    """
    ...
    perm_df = perm_df.sort_values("Importance", ascending=False).reset_index(drop=True)
    return perm_df


# ====================== SAC Environment (only if deps exist) ======================
if gym is not None and spaces is not None:
    class BiocharSACEnv(gym.Env):
        """
        Surrogate environment using model.predict().

        obs: 16 features normalized to [0,1]
        action: Top-K continuous deltas in [-1,1]
        ...
        def __init__(
            ...
            self.step_vec = self.base_step * (0.3 + 0.7 * self.imp_topk)
...
if model:
    ...
    with tab_single:
        ...
        if st.button("Predict Result", type="primary", use_container_width=True, key="btn_predict_single"):
            ...
            st.session_state.result = {
                "pred": pred_value,
                "shap": shap_values[0],
                "base": float(np.array(base_value).reshape(-1)[0]) if hasattr(base_value, "__len__") else float(base_value),
                "input": input_data,
            }
        ...
        if "result" in st.session_state:
                ...
                fig_gauge = go.Figure(
                    go.Indicator(
                        ...
                        gauge={
                            ...
                            "threshold": {"line": {"color": "red", "width": 4}, "thickness": 0.75, "value": res["pred"]},
                        },
                    )
                )
            ...
            with col_shap1:
                try:
                    ...
                    st.warning("SHAP plot not available for this model type.")

            with col_shap2:
                st.write("Feature Contributions:")
                shap_df = pd.DataFrame({"Feature": feature_names, "SHAP Value": res["shap"]})
    ...
    with tab_depend:
        ...
        with st.expander("⚙️ Advanced Settings (Resolution)"):
            points = st.slider("Curve Smoothness (Points)", 10, 200, 50, help="Higher values make the curve smoother.", key="sa_points")
        ...
        if st.button("Run Analysis", type="primary", key="sa_run_button"):
            try:
                ...
                temp_df = pd.DataFrame([base_input_dict] * points)[feature_names]
                ...
                y_list = np.asarray(y_pred).ravel().tolist()
    ...
    with tab_inter:
        ...
        with col_inter3:
            view_mode = st.radio("View Mode", ["2D Heatmap", "3D Surface"], horizontal=True, key="inter_view")
        ...
        if st.button("Generate Plot", type="primary", key="inter_btn"):
            try:
                ...
                batch_df = pd.DataFrame([base_input_dict] * (res_inter * res_inter))[feature_names]
                batch_df[feat_x] = X_flat
                batch_df[feat_y] = Y_flat

                Z_pred = model.predict(batch_df)
                Z_grid = np.asarray(Z_pred).reshape(res_inter, res_inter)
                ...
                max_idx = int(np.argmax(Z_pred))
                st.success(f"Max Qe ({float(np.asarray(Z_pred)[max_idx]):.2f}) at {feat_x}={X_flat[max_idx]:.2f}, {feat_y}={Y_flat[max_idx]:.2f}")
    ...
    with tab_opt:
        ...
        with col_opt1:
            target_qe = st.number_input("Target Qe (mg/g)", min_value=0.0, value=90.0, step=10.0, key="opt_target")
            n_iter = st.slider("Search Iterations (Monte Carlo)", 1000, 50000, 10000, key="opt_niter")
        ...
        with col_opt2:
            ...
            opt_features = st.multiselect("Features to Optimize", feature_names, default=default_opts, key="opt_feats")
            ...
            try:
                ...
                if len(success_df) > 0:
                    ...
                    with st.expander("📊 Solution Distribution Analysis", expanded=True):
                        st.write("Distribution of top 100 solutions for targeted features:")
    ...
    with tab_imp:
        ...
        st.info("Shuffle one feature and observe how much predictions change (global sensitivity).")

        n_samples = st.slider("Simulation Samples", 500, 5000, 1000, key="imp_samples")
        imp_seed = st.number_input("Seed", value=42, step=1, key="imp_seed")

        if st.button("Calculate Permutation Importance", type="primary", key="perm_imp_btn"):
            with st.spinner("Computing permutation importance (cached)..."):
                perm_df_desc = compute_permutation_importance_cached(feature_names, feature_ranges, int(n_samples), seed=int(imp_seed))

            # 保存给 RL Tab 复用
            st.session_state["perm_df"] = perm_df_desc

            # 画图：从小到大更好看（横向条形图）
            perm_df_plot = perm_df_desc.sort_values("Importance", ascending=True).reset_index(drop=True)

            fig_height = max(6, len(feature_names) * 0.4)
            fig, ax = plt.subplots(figsize=(10, fig_height))

            norm = plt.Normalize(perm_df_plot["Importance"].min(), perm_df_plot["Importance"].max())
            colors = plt.cm.Blues(norm(perm_df_plot["Importance"]))

            bars = ax.barh(perm_df_plot["Feature"], perm_df_plot["Importance"], color=colors, edgecolor="black", linewidth=0.5)
            for bar in bars:
                width = bar.get_width()
                ax.text(width * 1.01, bar.get_y() + bar.get_height() / 2, f" {width:.2f}", va="center", ha="left", fontsize=10, color="black")
            ...
            top_feature = perm_df_desc.iloc[0]["Feature"]
            st.success(f"💡 Most impactful feature: **{top_feature}**")

            csv_imp = perm_df_desc.to_csv(index=False).encode("utf-8")
            st.download_button("📥 Download Importance Data (CSV)", csv_imp, "permutation_importance.csv", "text/csv")

        st.markdown("</div>", unsafe_allow_html=True)

    # ======================= TAB 6: RL Optimization (SAC) =======================
    with tab_rl:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown("### 🤖 RL Optimization (SAC) — Maximize Qe with Auto Top-K by Importance")

        # 依赖检查
        if gym is None or SAC is None or DummyVecEnv is None or spaces is None:
        ...
        try:
            ...
            if isinstance(model, DummyRegressor):
                st.warning("Current model is DummyRegressor (demo mode). SAC training will not be meaningful. Please load XGBoost.pkl.")
        except Exception:
            pass
        ...
        with col_rl1:
            k_top = st.slider("Top-K Features (auto selected)", 2, min(10, len(feature_names)), 5, key="rl_k")
        with col_rl2:
            n_imp_samples = st.slider("Importance Samples", 300, 3000, 800, step=100, key="rl_imp_samples")
        with col_rl3:
            rl_seed = st.number_input("Random Seed", value=42, step=1, key="rl_seed")
        ...
        if perm_df is None or not isinstance(perm_df, pd.DataFrame) or len(perm_df) == 0:
            with st.spinner("Computing feature importance (cached)..."):
                perm_df = compute_permutation_importance_cached(feature_names, feature_ranges, int(n_imp_samples), seed=int(rl_seed))
        ...
        with c1:
            max_steps = st.slider("Episode Length (max_steps)", 10, 60, 25, key="rl_max_steps")
        with c2:
            total_steps = st.slider("Training Timesteps", 5_000, 200_000, 50_000, step=5_000, key="rl_total_steps")
        with c3:
            base_step = st.slider("Base Step (normalized)", 0.01, 0.20, 0.06, step=0.01, key="rl_base_step")
        with c4:
            lr = st.select_slider("Learning Rate", options=[1e-4, 3e-4, 1e-3], value=3e-4, key="rl_lr")

        p1, p2, p3 = st.columns(3)
        with p1:
            lam_change = st.slider("Change Penalty λ", 0.0, 0.50, 0.05, step=0.01, key="rl_lam_change")
        with p2:
            lam_edge = st.slider("Edge Penalty β", 0.0, 1.00, 0.20, step=0.05, key="rl_lam_edge")
        with p3:
            gamma = st.slider("Discount γ", 0.80, 0.999, 0.98, step=0.001, key="rl_gamma")

        adv = st.expander("Advanced SAC Hyperparameters", expanded=False)
        with adv:
            buffer_size = st.slider("Replay Buffer Size", 50_000, 500_000, 200_000, step=50_000, key="rl_buffer")
            batch_size = st.select_slider("Batch Size", options=[128, 256, 512], value=256, key="rl_batch")
            tau = st.slider("Target Smoothing τ", 0.001, 0.05, 0.02, step=0.001, key="rl_tau")

        run_train = st.button("🚀 Train SAC Agent", type="primary", use_container_width=True, key="rl_train_btn")
        ...
        if agent is not None and cfg is not None:
            ...
            eval_runs = st.slider("Evaluation Rollouts (pick best)", 1, 30, 5, key="rl_eval_runs")
            deterministic = st.checkbox("Deterministic Policy", value=True, key="rl_det")

            if st.button("📈 Run Evaluation Rollouts", type="primary", use_container_width=True, key="rl_eval_btn"):
                ...
                res_df = pd.DataFrame(
                    {"Feature": feature_names, "Start": x0, "Best": best_x, "Delta": (best_x - x0)}
                )
                ...
                if best_trace is not None:
                    fig_trace = go.Figure()
                    fig_trace.add_trace(go.Scatter(x=list(range(1, len(best_trace) + 1)), y=best_trace, mode="lines+markers", name="Qe trace"))
    ...
    with tab_compare:
        ...
        with col_scen_b:
            ...
            with st.expander("Configure Scenario B", expanded=True):
                for idx, (feature, props) in enumerate(feature_ranges.items()):
                    ...
                    if default_val > float(props["max"]):
                        default_val = float(props["max"])
                    val = st.number_input(
                        f"B: {feature}",
                        min_value=float(props["min"]),
                        max_value=float(props["max"]),
        ...
        if st.button("Compare Scenarios", type="primary", use_container_width=True, key="compare_btn"):
            ...
            fig_radar = go.Figure()
            fig_radar.add_trace(go.Scatterpolar(r=norm_a_closed, theta=features_closed, fill="toself", name="Scenario A", line_color="#3498db"))
            fig_radar.add_trace(go.Scatterpolar(r=norm_b_closed, theta=features_closed, fill="toself", name="Scenario B", line_color="#e74c3c"))
    ...
    with tab_batch:
        ...
        st.download_button("📥 Download Template CSV", data=csv_template, file_name="template.csv", mime="text/csv", key="batch_tpl")

        uploaded_file = st.file_uploader("Upload your data file", type=["csv", "xlsx"], key="batch_upload")
